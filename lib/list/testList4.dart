// ini sama dengan file testList hanya mau test pake state less
//karena yg di statfull sepertinya merender 2 kali pertama kali jalan
...
class ListBasicstatetfull extends StatelessWidget {

  final orang = List();
  final List<Widget> orang2 = [];

  ListBasicstatetfull(){
    for (int i = 0; i < 20 ; i++) {
      orang.add({'nama': 'boneka' + i.toString(), 'hobi': 'bohong' + i.toString()});
      orang2.add(TemplateList(title: 'user ke' + i.toString(), subtitle: 'hobi nya :'+ i.toString()));
    }
  }
...
//    ];
  ...
  final listItems = ambilDataList();

  @override
  Widget build(BuildContext context) {
//    debugPrint(listItems.toString());
//    debugPrint(orang.toString());
    debugPrint(orang2.toString());
    return Scaffold(
      appBar: AppBar(
